...
(defn consent-scopes
  ...
  []
  (str/split (or (consent-param "scope") "") #"\s+"))

(defn consent-has-no-cloud-scope
  "No Cloud Platform scope on the consent screen. That is the whole bean: one
   machine scope on a human's grant puts Gmail, Chat and the directory under
   a 16-hour reauthentication clock (isaac-ey6q, isaac-286x)."
  []
  (let [scopes (consent-scopes)]
    (g/should (seq scopes))
    (doseq [scope scopes]
      (g/should-not (str/includes? scope "auth/pubsub"))
      (g/should-not (str/includes? scope "cloud-platform"))
      (g/should-not (str/includes? scope "auth/devstorage"))
      (g/should-not (str/includes? scope "auth/cloud-")))))

(defn consent-asks-for-scope [scope]
